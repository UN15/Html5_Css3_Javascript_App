...
bookForm.addEventListener('submit', function (e) {
    ...
    const bookData = {
        ...
        detailRequest: {
            ...
            pageCount: formData.get('pageCount') ? parseInt(formData.get('pageCount')) : 0,
...
function validateBook(book) {
    ...
    // 페이지 수 유효성 검사
    if (book.detailRequest.pageCount !== null) {
        if (isNaN(book.detailRequest.pageCount)) {
            alert('페이지 수는 숫자여야 합니다.');
            return false;
        }
    ...
    // URL 형식 검사 (입력된 경우에만)
    if (book.detailRequest.coverImageUrl !== null) {
        if (!isValidUrl(book.detailRequest.coverImageUrl)) {
...
function deleteBook(bookId){
    if(!confirm(`ID = ${bookId}인 학생을 정말로 삭제하시겠습니까?`)){
        return;
    }
...
function editBook(bookId) {
    fetch(`${API_BASE_URL}/api/books/${bookId}`)
        ...
        .then((book) => {
            ...
            //버튼의 타이틀을 등록 => 수정으로 변경
            submitButton.textContent = "도서 정보 수정";
            //취소 버튼을 활성화
